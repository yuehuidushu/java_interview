![img.png](images/redis缓存/缓存相关总结.png)
# 1. 缓存雪崩
...
那么，当**[大量缓存数据在同一时间过期（失效）或者 Redis 故障宕机]**时，如果此时有大量的用户请求，
...
![img.png](images/redis缓存/缓存击穿.png)
...
在数据库写入数据 x 后，把数据 x 标记在布隆过滤器时，数据 x 会被 3 个哈希函数分别计算出 3 个哈希值，
